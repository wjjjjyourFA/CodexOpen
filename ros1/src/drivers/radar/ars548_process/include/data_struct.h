...
typedef struct _RadarObjectList
{
    ...
    RadarObject Objects[50];
}RadarObjectList;

// for 5.48.4
typedef struct _RadarBasicStatus
{
    float Longitudinal;  // 4
    float Lateral;
    float Vertical;
    float Yaw;
    float Pitch;
...
typedef struct _RadarStatus
{
    ...
    unsigned char Status_CharacteristicSpeed;  // 1
    unsigned char Status_RadarStatus;  // Status_Radar;
...
typedef struct _RadarConfiguration
{
    RadarBasicStatus BasicConf;
