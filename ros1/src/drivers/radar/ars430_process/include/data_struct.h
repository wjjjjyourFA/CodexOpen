...
//float RDI Data ==> get point(x y z)
...
typedef struct SSPacket {
  ...
  uint64_t    UTCTimeStamp;
  uint32_t    TimeStamp;
  uint32_t    SurfaceDamping_Raw; // ==> CurrentDamping
  ...
  uint8_t     Defective;
  uint8_t     BadSupplyVolt;  // ==> SupplVoltLimit
  uint8_t     BadTemp;  // ==> SensorOffTemp
  uint8_t     GmMissing;

  // uint8_t     TxPowerStatus;
  uint8_t     TxOutReduced;
...
typedef struct PacketGroup {
  ars430_process::RadarPacket nearPackets[NUM_NEAR];
  ars430_process::RadarPacket farPackets[NUM_FAR];
