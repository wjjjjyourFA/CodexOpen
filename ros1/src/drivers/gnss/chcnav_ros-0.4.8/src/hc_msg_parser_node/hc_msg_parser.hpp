 ...
 * @brief 供解析器使用的回调函数
 * */
static int parser_read_handler(void *buffer, size_t size, size_t *size_read,
                               void *parser_id);
 ...
 * @return int
 * */
static int msg_recv_callback(hc__single_msg_t single_msg,
                             hc__msg_parser_node *node);
...
class hc__msg_parser_node {
 ...
 public:
  ...
  ros::Publisher gnss_puber;  // gps topic
  ...
  RacGnssReceiver *rac_gnss_receiver;
 ...
 public:
  /**
     ...
     * */
  hc__msg_parser_node(unsigned int hz, hc__device_connector *device_connector,
                      ros::NodeHandle *nh, ros::NodeHandle *private_nh,
                      string node_name, std::shared_ptr<RuntimeConfig> param) {
    if (NULL != nh && NULL != private_nh && NULL != device_connector) {
      this->nh         = nh;
      this->private_nh = private_nh;

      std::string ns    = param->GetVehicleName();  // 获取车辆名称
      std::string topic = "/" + ns + param->channel_name;

      // 初始化nmea协议话题
      // a public topic
      this->nmea_puber =
          this->nh->advertise<chcnav::string>(topic + "/nmea_sentence", 1000);
      this->nmea_msg.header.seq      = 0;
      this->nmea_msg.header.frame_id = param->frame_id;

      // a public topic
      this->gnss_puber = this->nh->advertise<custom_sensor_msgs::Gnss>(
          topic + "/gps_data", 1000);
      this->gnss_msg.header.seq      = 0;
      this->gnss_msg.header.frame_id = param->frame_id;

      // 初始化华测自定义协议话题
      this->hc_puber = this->nh->advertise<chcnav::hc_sentence>(
          topic + "/hc_sentence", 1000);  // a public topic
      this->hc_msg.header.seq      = 0;
      this->hc_msg.header.frame_id = param->frame_id;

      // 订阅写话题
      this->write_suber = this->private_nh->subscribe(
          topic + "/write", 20, &hc__msg_parser_node::node_write, this);

      // 初始化解析器
      hc__init_msg_parser(&this->parser, HC_PROTOCOL_MAX_LEN,
                          parser_read_handler, this);
      // 在这里将抓取TCP数据的函数方式传入 => this->parser

      //
      rac_gnss_receiver = new RacGnssReceiver();
  ...
  void node_start(void) {
    ...
    // 此处循环表示直到成功取到一次数据（指通过校验，hc cgi 自定义协议或者 nmea 协议）后才 break 该层循环进入 sleep
    while (ros::ok())  
    {
      while (1) {
        ...
        switch (token.type) {
          case HC__TOKERN_HC_SHORT_MSG:
            ...
            single_msg.type_value.cgi_header.msg_id =
                (this->token.token_value.value[7] << 8) +
                this->token.token_value.value[6];
            memcpy(single_msg.value, this->token.token_value.value,
                   this->token.token_value.value_len);
          ...
          case HC__TOKERN_NMEA_MGS:
            ...
            if (this->token.token_value.value[6] == ',')
              memcpy(single_msg.type_value.nmea.header,
                     this->token.token_value.value + 1, 5);
            else
              memcpy(single_msg.type_value.nmea.header,
                     this->token.token_value.value + 1, 6);
          ...
          default:
            ...
            memset(parser.error.description, 0x00,
                   sizeof(parser.error.description));
            parser.state = HC__STATE_S0;
            break;
        }

        // 直到取到了一条合法消息才会休眠
        if (token.type == HC__TOKERN_HC_MSG ||
  ...
  ~hc__msg_parser_node() {
    this->device_connector->disconnect();
    delete this->rac_gnss_receiver;
  }
};

int parser_read_handler(void *buffer, size_t size, size_t *size_read,
                        void *parser_id) {
...
int msg_recv_callback(hc__single_msg_t single_msg, hc__msg_parser_node *node) {
  // single_msg 已经是一个完整的帧消息，直接对其进行处理
  int index;
  ...
  switch (single_msg.msg_type) {
    case HC__MSG_NMEA:
      if (single_msg.value_len < 5) return -1;
      ...
      if (strtol(raw_xor_string, NULL, 16) != xor_check_uchar) {
        ...
        // 我需要在这里加上 nmea 转 self globalpose 的处理函数吗？
        // 是否会导致处理频率不够？

        // printf("node->nmea_msg: %s\n", node->nmea_msg.sentence.c_str());
        node->rac_gnss_receiver->SetMsg(&node->gnss_msg /*it will be changed*/);
        node->rac_gnss_receiver->Resolve(&node->nmea_msg);
        node->gnss_puber.publish(node->gnss_msg);
        node->rac_gnss_receiver->ResetMsg();
      }
      break;
