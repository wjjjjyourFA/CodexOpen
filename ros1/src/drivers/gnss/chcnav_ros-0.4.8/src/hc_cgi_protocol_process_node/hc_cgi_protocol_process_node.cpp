#include <hc_cgi_protocol_process_node.hpp>

GnssChannel::GnssChannel(ros::NodeHandle &parent_nh,
                         const std::string &topic_ns) {
  // 创建新的 CallbackQueue
  callback_queue = new ros::CallbackQueue();

  // 拷贝 NodeHandle，并设置其使用自定义 queue
  nh = ros::NodeHandle(parent_nh);
  nh.setCallbackQueue(callback_queue);

  // 设置订阅 & 发布
  sub = nh.subscribe(topic_ns + "/hc_sentence", 1000,
                     &GnssChannel::hc_sentence_callback, this);
  gs_devpvt_pub =
      nh.advertise<chcnav::hcinspvatzcb>(topic_ns + "/devpvt", 1000);
  gs_devimu_pub = nh.advertise<chcnav::hcrawimub>(topic_ns + "/devimu", 1000);

  // 启动 AsyncSpinner 使用该 queue
  spinner = new ros::AsyncSpinner(1, callback_queue);
  spinner->start();
}

GnssChannel::~GnssChannel() {
  if (spinner) {
    spinner->stop();
    delete spinner;
  }
 ...
 * @param msg 接收到的数据
 * */
void GnssChannel::hc_sentence_callback(
    const chcnav::hc_sentence::ConstPtr &msg) {
...
void GnssChannel::msg_deal__hcinspvatzcb(
    const chcnav::hc_sentence::ConstPtr &msg) {
  ...
  if (msg->data.size() == 296) {
    ...
    devpvt.header.stamp = ros::Time(devpvt.week * 7.0 * 24.0 * 3600.0 +
                                    devpvt.second + 315964800.0 - g_leaps);
    ...
    devpvt.vehicle_linear_acceleration_without_g.x =
        *((float *)(&msg->data[128]));
    devpvt.vehicle_linear_acceleration_without_g.y =
        *((float *)(&msg->data[132]));
    devpvt.vehicle_linear_acceleration_without_g.z =
        *((float *)(&msg->data[136]));
...
void GnssChannel::msg_deal__hcrawimuib(
    const chcnav::hc_sentence::ConstPtr &msg) {
  ...
  if (msg->data.size() == 68) {
    ...
    devimu.header.stamp = ros::Time(devimu.week * 7.0 * 24.0 * 3600.0 +
                                    devimu.second + 315964800.0 - g_leaps);
  ...
  if (msg->data.size() == 34) {
    ...
    // xyz角角速度 g
    devimu.angular_acceleration.x =
        ((float)*((short *)(&msg->data[20]))) / 5000.0;
    devimu.angular_acceleration.y =
        ((float)*((short *)(&msg->data[22]))) / 5000.0;
    devimu.angular_acceleration.z =
        ((float)*((short *)(&msg->data[24]))) / 5000.0;
...
int main(int argc, char **argv) {
  ...
  std::string name =
      param_manager->GetVehicleName() + "_hc_cgi_protocol_process_node";
  ...
  // 初始化多个Gnss Process
  int num = 1;
  std::vector<std::unique_ptr<GnssChannel>> channels;
  for (auto config : gnss_configs) {
    ...
    int current_num = num;
    // 启动 单线程/多线程
    channels.emplace_back(std::make_unique<GnssChannel>(nh, topic));
