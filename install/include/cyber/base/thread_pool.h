...
class ThreadPool {
 public:
  explicit ThreadPool(std::size_t thread_num = 4, std::size_t max_task_num = 1000);
...
inline void ThreadPool::Wait() {
  std::unique_lock<std::mutex> lock(wait_mutex_);
  wait_cv_.wait(lock, [this]() { return active_tasks_.load() == 0; });
}
