...
// AABB 只能表示边界框与坐标轴对齐的情况。
// 如果物体发生旋转，AABB 的边界会包含旋转后物体的最小外接矩形，但不能准确表示其真实形状。
template <typename T>
struct Rect {
  ...
  jojo::perception::base::Point2D<T> Center() const {
    jojo::perception::base::Point2D<T> p;
    p.x = this->x + this->width / 2.0;
    p.y = this->y + this->height / 2.0;
    return p;
  }

  void SetCenter(jojo::perception::base::Point2D<T> p) {
...
// 基于角点的方式，即通过边界框的左上角 (xmin, ymin) 和右下角 (xmax, ymax) 来表示一个二维边界框。
template <typename T>
struct BBox2D {
  ...
  jojo::perception::base::Point2D<T> Center() const {
    jojo::perception::base::Point2D<T> p;
    p.x = this->xmin + (this->xmax - this->xmin) / 2.0;
    p.y = this->ymin + (this->ymax - this->ymin) / 2.0;
    return p;
  }
