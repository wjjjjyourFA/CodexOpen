...
// from ros-noetic-sophus
// #include "sophus/so3.hpp"
// from robosense
#include "third_party/Sophus/sophus/so3.h"
...
inline Eigen::Matrix<double, 24, 24> df_dx(state_ikfom &s, const input_ikfom &in) {
  ...
  // 对应公式(7)第3行第1列
  cov.template block<3, 3>(12, 3) = -s.rot.matrix() * Sophus::SO3d::hat(acc_);
  // 对应公式(7)第3行第5列
  cov.template block<3, 3>(12, 18) = -s.rot.matrix();
  // 对应公式(7)第3行第6列 I
...
inline Eigen::Matrix<double, 24, 12> df_dw(state_ikfom &s, const input_ikfom &in) {
  ...
  // 对应公式(7)第3行第2列 -R
  cov.template block<3, 3>(12, 3) = -s.rot.matrix();
