...
#include "modules/common/math/euler_angles_xyz.h"
...
inline double QuaternionToHeading(const double qw, const double qx,
  ...
  EulerAnglesXYZd euler_angles(qw, qx, qy, qz);
  // euler_angles.yaw() is zero when the car is pointing North, and
  // the heading is zero when the car is pointing East.
  // return NormalizeAngle(euler_angles.yaw() + M_PI_2);

  // euler_angles.yaw() is zero when the car is pointing East, and
  // the heading is zero when the car is pointing East.
...
inline Eigen::Quaternion<T> HeadingToQuaternion(T heading) {
  ...
  // Note that heading is zero at East and yaw is zero at East.
  EulerAnglesXYZ<T> euler_angles(heading);
...
// clang-format off
/*
inline Eigen::Vector3d InverseQuaternionRotate(const Quaterniond &orientation,
