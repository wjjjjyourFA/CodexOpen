...
class CameraDevice : public QObject {
 ...
 public:
  ...
  FrameQueue<cv::Mat>::Stats queueStats() const {
    return queue_ ? queue_->stats() : FrameQueue<cv::Mat>::Stats{};
  }
 ...
 protected:
  ...
  bool openNewWriter(int segmentSeconds = 60 * 60);  // default 1 hour
 ...
 private:
  ...
  // 自动录制切分功能
  int segmentSeconds = 3600;  // 默认每段录像 1 小时
