...
void CameraDevice::startRecording() {
  ...
  // qDebug() << "[CameraDevice] Recording started:" << devKey << segmentSeconds;
}
...
void CameraDevice::startAutoRecording() {
  ...
  // encode
  quit_flag_encode = false;
  this->openNewWriter(this->segmentSeconds);
  writerThread = QThread::create([this] { encode(); });
  writerThread->start();
...
void CameraDevice::openNewWriter(int segmentSeconds) {
  if (segmentSeconds <= 0) return;

  // 具体的文件
  QString filePath = generateVideoFileRoot();

  writer = std::make_shared<FFmpegWriter>(config_->width(), config_->height(),
                                          config_->frame_rate(), segmentSeconds,
...
void CameraDevice::stopRecording() {
  if (isRecording()) {
    ...
    if (writerThread) {
      ...
      // 由主线程统一关闭
      if (writer) {  // 等价于 writer != nullptr
        writer->reset();  // 自动 close / flush / rename
      }
    }
...
void CameraDevice::encode() {
  ...
  while (!quit_flag_encode) {
    ...
    if (queue_->pop(frame)) {
      ...
      int ret = writer->write(frame);
      // 直接发送信号给主窗口，用于跳出信号。
      if (ret > 0) {
        emit recordError(devKey, ret);
      }
      if (writer->rotateIfNeeded()) {
        recordTimer.restart();
      }
      // auto writeEnd = std::chrono::steady_clock::now();
    } else {
      QThread::msleep(5);  // 队列空，避免空转
    }
