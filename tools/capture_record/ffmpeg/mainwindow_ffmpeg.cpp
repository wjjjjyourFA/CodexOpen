...
void MainWindow::initSerial() {
  ...
  // for rs232 to ttl ==> COM1
  try {
#if defined(CAPTURE_RM1000)
    serialPtr = std::make_unique<SerialRM1000>(8, 1);
#else
    serialPtr = std::make_unique<SerialRS232>("/dev/ttyS0", 115200);
#endif
  } catch (const std::exception& e) {
    QMessageBox::critical(this, "Serial Error", e.what());
    QTimer::singleShot(0, qApp, &QCoreApplication::quit);
    return;
  }
}

void MainWindow::trySerial() {
  this->initSerial();
...
void MainWindow::startCamera(const QString& devKey, int mode) {
  ...
  CameraDevice* cam     = new CameraDevice(device_name, customSettings, this);
  ...
  connect(cam, &CameraDevice::previewAvailable, this,
          &MainWindow::processCapturedImage, Qt::QueuedConnection);
  ...
  connect(cam, &CameraDevice::recordError, this, &MainWindow::updateRecordError);
  // clang-format on
...
void MainWindow::startAllRecording() {
  ...
  // 触发一次：高电平 100ms
  // this->serialPtr->triggerHigh(trigger_duration);
  /* 再触发一次，确保触发成功
  // usleep(100);
  // this->serialPtr->triggerHigh(trigger_duration);
  */
...
void MainWindow::startAllAutoRecording() {
  ...
  // 60000 每分钟检查一次；10000 每10s检查一次
  dailyRecordTimer->start(10000);
  // 启动时立刻检查一次
...
void MainWindow::updateRecordTime(const QString& devKey,
  ...
  messages << QString("RecordTime: %1: %2").arg(devKey).arg(timeStr);
  ui->statusbar->showMessage(messages.join(" | "));
}

void MainWindow::updateRecordError(const QString& devKey, int errorCode) {
  // 构造基础信息
  QString msg;
  // 根据 errorCode 处理不同类型
  switch (errorCode) {
    case 1:
      msg = QString("Camera %1: Pipe is null").arg(devKey);
      break;
    case 2:
      msg = QString("Camera %1: FFmpeg write failed").arg(devKey);
      break;
    case 3:
      msg = QString("Camera %1: Not enough disk space (minimum %2 GB required)")
                .arg(devKey)
                .arg(10);
      break;
    case 4:
      msg = QString("Camera %1: Failed to start or rotate FFmpeg").arg(devKey);
      break;
    case 5:
      msg =
          QString("Camera %1: Invalid frame format or dimensions").arg(devKey);
      break;
    default:
      msg = QString("Camera %1: Unknown error (%2)").arg(devKey).arg(errorCode);
      break;
  }
  ...
  // 如果弹窗已经显示过，就先缓存
  if (recordErrorPopupShown_) {
    pendingErrors_ << msg;
    return;
  }

  // 第一次弹窗
  recordErrorPopupShown_ = true;

  // 使用 QTimer::singleShot 弹窗，避免阻塞 UI
  QTimer::singleShot(0, this, [this, msg]() {
    QMessageBox::warning(this, "Camera Record Error", msg);

    // 弹窗关闭后，如果有缓存的错误，再显示一次
    if (!pendingErrors_.isEmpty()) {
      QString combined = pendingErrors_.join(" | ");
      pendingErrors_.clear();

      QMessageBox::warning(this, "Camera Record Error", combined);
    }

    // 重置标志位
    recordErrorPopupShown_ = false;
  });
}
...
void MainWindow::refreshRecorderControls() {
  ...
  ui->recordButton->setEnabled(((!anyRecording && !anyAutoRecording) || anyPaused || anyError) && !anyTransitioning && !isAutoRecording);

  // - 自动录制按钮可用条件：没有自动录制正在进行，并且手动录制没有在跑，或者当前是暂停状态
  ui->autoRecordButton->setEnabled(((!anyAutoRecording && !anyRecording) || anyPaused || anyError) && !anyTransitioning && !isAutoRecording);

  // pauseButton：如果至少一个相机正在录制，则可用
  ui->pauseButton->setEnabled((anyRecording || anyAutoRecording) && !anyTransitioning);

  // stopButton：如果至少有一个相机正在录制或暂停，则可用
  ui->stopButton->setEnabled((((anyRecording && !anyAutoRecording) || anyPaused || anyError)) && !anyTransitioning && !isAutoRecording);

  ui->autoStopButton->setEnabled(!anyTransitioning && ((anyAutoRecording && !anyRecording) || anyPaused || anyError || isAutoRecording));
  // clang-format on
...
void MainWindow::onActionSetSavePlan() {
  ...
  // QTimeEdit *startTimeEdit = new QTimeEdit(QTime::fromString("08:00", "HH:mm"), this);
  ...
  // QTimeEdit *intervalTimeEdit = new QTimeEdit(QTime::fromString("00:30", "HH:mm"), this);
  ...
  // QTimeEdit *endTimeEdit = new QTimeEdit(QTime::fromString("20:00", "HH:mm"), this);
  ...
  if (dlg.exec() == QDialog::Accepted) {
    ...
    if (intervalSec <= 0) {
      QMessageBox::warning(this, tr("参数错误"), tr("录制间隔必须大于 0"));
      return;
    }
    ...
    customSettings.crossEndSec        = endSec;
...
void MainWindow::checkDailyRecordStatus() {
  ...
  const bool shouldAutoRecord =
      isWithinDailyRecordWindow(nowSec, customSettings.recordStartSeconds,
                                customSettings.recordEndSeconds);
