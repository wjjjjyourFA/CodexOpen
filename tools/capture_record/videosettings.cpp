...
#include "tools/capture_record/pro/ui/ui_videosettings.h"
...
VideoSettings::VideoSettings(QMediaRecorder* mediaRecorder, QWidget* parent)
  ...
  foreach (int sampleRate, mediaRecorder->supportedAudioSampleRates()) {
    ui->audioSampleRateBox->addItem(QString::number(sampleRate),
                                    QVariant(sampleRate));
  }
  ...
  foreach (const QString& format, mediaRecorder->supportedContainers()) {
    ui->containerFormatBox->addItem(
        format + ":" + mediaRecorder->containerDescription(format),
        QVariant(format));
  }
}
...
QAudioEncoderSettings VideoSettings::audioSettings() const {
  ...
  settings.setQuality(
      QMultimedia::EncodingQuality(ui->audioQualitySlider->value()));
  settings.setSampleRate(boxValue(ui->audioSampleRateBox).toInt());
  return settings;
}

void VideoSettings::setAudioSettings(
    const QAudioEncoderSettings& audioSettings) {
  selectComboBoxItem(ui->audioCodecBox, QVariant(audioSettings.codec()));
  selectComboBoxItem(ui->audioSampleRateBox,
                     QVariant(audioSettings.sampleRate()));
...
QVideoEncoderSettings VideoSettings::videoSettings() const {
  ...
  settings.setQuality(
      QMultimedia::EncodingQuality(ui->videoQualitySlider->value()));
...
void VideoSettings::setVideoSettings(
  ...
  selectComboBoxItem(ui->videoResolutionBox,
                     QVariant(videoSettings.resolution()));
