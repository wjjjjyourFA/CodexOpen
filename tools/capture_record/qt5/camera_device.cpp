...
void CameraDevice::startRecording() {
  ...
  if (filePath.isEmpty()) {
    emit recorderError(devKey, tr("无法创建录像目录"));
    return;
  }
  ...
  if (!recorder->setOutputLocation(QUrl::fromLocalFile(filePath))) {
    emit recorderError(devKey, tr("无法设置录像输出路径: %1").arg(filePath));
    return;
  }
  recorder->record();
  // */
...
QString CameraDevice::generateVideoFilePath() {
  ...
  QDir dir;
  if (!dir.exists(fullDirPath) && !dir.mkpath(fullDirPath)) {
    qWarning() << "[CameraDevice] Failed to create directory:" << fullDirPath;
    return {};
  }
