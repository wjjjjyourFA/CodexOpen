...
void CameraDevice::close() {
  if (isRecording() || writerThread || writer.isOpened()) stopRecording();
...
void CameraDevice::stopRecording() {
  if (recording.load(std::memory_order_acquire) == RecordState::Stopped &&
      !writerThread && !writer.isOpened()) return;
...
QString CameraDevice::generateVideoFilePath(const std::string& suffix) {
  ...
  const QString filePath = generateUniqueMediaFilePath(
      fullDirPath, QString::fromStdString(suffix));
  if (filePath.isEmpty()) {
