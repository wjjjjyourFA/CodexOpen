...
{
    std::shared_ptr<image_dds_type> image_msg_ptr = std::make_shared<image_dds_type>();
    ...
    memcpy(image_msg_ptr->data().data(), image.data, image_msg_ptr->data().size() * sizeof(uint8_t));

    return image_msg_ptr;
}

std::shared_ptr<compressed_image_dds_type> CvImage::toCompressedImageMsg(int compress_quality) const
{
    std::shared_ptr<compressed_image_dds_type> compressed_image_msg_ptr = std::make_shared<compressed_image_dds_type>();
    ...
    // 压缩图像
    cv::imencode(".jpg", image, compressed_image_msg_ptr->data(), compression_params);
                 ...
                 const std::string& encoding)
{
    ...
    const cv::Mat recv_img(source.height(), source.width(), type, image_data.data());

    CvImagePtr cv_image_ptr = std::make_shared<CvImage>(source.header(), source.encoding(), std::move(recv_img.clone()));
                    ...
                    const std::string& encoding)
{
    ...
    CvImagePtr cv_image_ptr = std::make_shared<CvImage>(source.header(), "bgr8", decode_img);
