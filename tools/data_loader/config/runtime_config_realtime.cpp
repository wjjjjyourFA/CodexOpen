...
void RuntimeConfigRealtime::LoadConfig(const std::string& config_path) {
  try {
    ...
    topic_infra_pub = ReadStringArray(pt, "topics.topic_infra_pub_", b_star);
    topic_star_pub = ReadStringArray(pt, "topics.topic_star_pub_", b_infra);

    topic_radar_pub = pt.get<std::string>("topics.topic_radar_pub", "");
    topic_radar_4d_pub = ReadStringArray(pt, "topics.topic_radar_4d_pub_", b_radar_4d);
