...
class CharucoCalibrator:
    def __init__(self, aruco_dict: int, squares_horizontally: int, squares_vertically: int, square_length: float, marker_length:float, root_path:str = None):
        ...
        self.board = cv2.aruco.CharucoBoard((self.squares_horizontally, self.squares_vertically), self.square_length, self.marker_length, self.aruco_dict)
        # opencv 4.6
    ...
    def generate_charuco_board(self) -> None:
        ...
        charuco_img = cv2.aruco.drawPlanarBoard(self.board, (img_width, img_height), marginSize=7, borderBits=1)
    ...
    def show_aruco_markers(self, image: np.ndarray, image_name:str = "Detected ArUco Markers", window_size = (480,480), verbose=True) -> None:
        """
        ...
        if marker_ids is not None and len(marker_ids) > 0:
            ...
            # 显示结果图像
            cv2.imshow(image_name, cv2.resize(image_marked, window_size))
            cv2.waitKey(1)
    ...
    def calibrate(self, grayscale = True, calibration_filename ='fisheye_calibration.json', window_size=(480,480), verbose=False) -> None:
        ...
        for image_name, image in self.calibration_images():
            ...
            if charuco_ids is not None and len(charuco_ids) > 4:
                ...
                # self.show_aruco_markers(image, image_name=f"{image_name}", window_size=window_size, verbose=verbose)
                # cv2.waitKey(0)
        ...
        self.calib_file_path = output_path + f"fisheye_calibration.json"
