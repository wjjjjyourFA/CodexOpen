#include "modules/tools/sensor_calibration/camera_intrinsic/intrinsic_calib/intrinsic_calibration.hpp"

namespace tools = jojo::tools;
namespace common = apollo::cyber::common;

bool IntrinsicCalibration::Init(const std::string& root_path,
                                const int& grid_size,  // in milimeter 50mm
                                const int& corner_width,
                                const int& corner_height) {
  if (!SetImageFiles(root_path)) {
    return false;
  }

  hps_.grid_size  = grid_size;
  hps_.board_size = cv::Size(corner_width, corner_height);

  return true;
}

bool IntrinsicCalibration::Init(std::shared_ptr<tools::RuntimeConfig> param) {
  param_ = param;

  if (!this->Init(param_->data_root_path, param_->grid_size,
                  param_->corner_width, param_->corner_height)) {
    return false;
  }

  // clang-format off
  AutoImagePickerHyperparams picker_hps;
  picker_hps.IMAGE_MARGIN_PERCENT                      = param_->IMAGE_MARGIN_PERCENT;
  picker_hps.CHESSBOARD_MIN_AREA_PORTION               = param_->CHESSBOARD_MIN_AREA_PORTION;
  picker_hps.CHESSBOARD_MIN_ANGLE                      = param_->CHESSBOARD_MIN_ANGLE;
  picker_hps.CHESSBOARD_MIN_MOVE_THRESH                = param_->CHESSBOARD_MIN_MOVE_THRESH;
  picker_hps.CHESSBOARD_MIN_ANGLE_CHANGE_THRESH        = param_->CHESSBOARD_MIN_ANGLE_CHANGE_THRESH;
  picker_hps.CHESSBOARD_MIN_AREA_CHANGE_PARTION_THRESH = param_->CHESSBOARD_MIN_AREA_CHANGE_PARTION_THRESH;
  picker_hps.MAX_SELECTED_SAMPLE_NUM                   = param_->MAX_SELECTED_SAMPLE_NUM;
  picker_hps.REGION_MAX_SELECTED_SAMPLE_NUM            = param_->REGION_MAX_SELECTED_SAMPLE_NUM;
  image_selector.updateHyperparams(picker_hps);
  // clang-format on

  return true;
}

bool IntrinsicCalibration::SetImageFiles(const std::string& root_path) {
  file_names.clear();
  // read image files
  if (root_path.rfind('/') != root_path.size() - 1) {
    img_dir_path_ = root_path + "/calib_images/";
  } else {
    img_dir_path_ = root_path + "calib_images/";
  }

  // set output path
  undistort_image_path_ = root_path + "/undistorted/";
  selected_image_path_  = root_path + "/selected/";
  detected_image_path_  = root_path + "/detected/";
  // std::cout << "image path: " << img_dir_path_ << std::endl;

  DIR* dir;
  if ((dir = opendir(img_dir_path_.c_str())) == NULL) {
    std::cerr << "[ERROR]Open Image Folder Failed." << std::endl;
    exit(1);
  }

  struct dirent* ptr;
  while ((ptr = readdir(dir)) != NULL) {
    const std::string entry_path = img_dir_path_ + ptr->d_name;
    struct stat entry_stat {};
    if (strcmp(ptr->d_name, ".") != 0 && strcmp(ptr->d_name, "..") != 0 &&
        stat(entry_path.c_str(), &entry_stat) == 0 &&
        !S_ISDIR(entry_stat.st_mode)) {
      file_names.push_back(ptr->d_name);
    }
  }

  if (file_names.size() == 0) {
    std::cerr << "[ERROR]No Calibration Image Files.";
    closedir(dir);
    return false;
  }
  std::sort(file_names.begin(), file_names.end());
  closedir(dir);

  /*
  if (opendir(selected_image_path_.c_str()) == nullptr) {
    char command[1024];
    sprintf(command, "mkdir -p %s", selected_image_path_.c_str());
    system(command);
    printf("Create dir: %s\n", selected_image_path_.c_str());
  }
  */
  common::CreateDir(selected_image_path_);
  common::CreateDir(detected_image_path_);

  // set calib result file
  calib_result_file_ = root_path + "/camera_params.yaml";
  // std::cout << calib_result_file_ << std::endl;
  common::CreateFile(calib_result_file_);

  return true;
}

bool IntrinsicCalibration::Calibrate() {
  selected_file_names.clear();

  std::vector<cv::Point3f> object_corners;
  object_corners.reserve(hps_.board_size.width * hps_.board_size.height);
  // generate 3D corner points for 单应性矩阵
  // Z = 0 的 标定板坐标系的 三维平面点
  // 生成 3D 标定板点，单位 mm
  for (int i = 0; i < hps_.board_size.height; i++) {
    for (int j = 0; j < hps_.board_size.width; j++) {
      object_corners.emplace_back(
          cv::Point3f(j * hps_.grid_size, i * hps_.grid_size, 0.0f));
    }
  }
  /* debug
  std::cout << "==== Object Corners ====" << std::endl;
  std::cout << "grid_size: " << hps_.grid_size << std::endl;
  std::cout << "hps: " << hps_.board_size.width << " x "
            << hps_.board_size.height << std::endl;
  for (int i = 0; i < hps_.board_size.height; i++) {
    for (int j = 0; j < hps_.board_size.width; j++) {
      int idx       = i * hps_.board_size.width + j;
      const auto& p = object_corners[idx];
      std::cout << "pt[" << idx << "] = (" << p.x << ", " << p.y << ", " << p.z
                << ")" << std::endl;
    }
  }
  */

  // pick image
  cv::Mat init_img = cv::imread(img_dir_path_ + file_names[0], 0);
  if (init_img.empty()) {
    std::cerr << "[ERROR]Failed to read calibration image: "
              << img_dir_path_ + file_names[0] << std::endl;
    return false;
  }
  img_size_ = init_img.size();

  image_selector.init(img_size_.width, img_size_.height, hps_.board_size.width,
                      hps_.board_size.height);
  std::cout << "auto picker is done..." << std::endl;

  // detect chessboard corner
  int total_image_num = file_names.size();
  // successfully corner-detected image num
  int detected_image_num = 0;
  for (size_t i = 0; i < file_names.size(); i++) {
    cv::Mat input_image;
    // 灰度读取
    // input_image = cv::imread(img_dir_path_ + file_names[i], 0);
    // 彩色读取
    input_image = cv::imread(img_dir_path_ + file_names[i], cv::IMREAD_COLOR);
    if (input_image.empty()) continue;
    // img_size_ = input_image.size();

    cv::Mat gray;
    cv::cvtColor(input_image, gray, cv::COLOR_BGR2GRAY);

    // detect corner
    std::vector<cv::Point2f> image_corners;
    bool whether_found = cv::findChessboardCorners(
        gray, hps_.board_size, image_corners,
        cv::CALIB_CB_ADAPTIVE_THRESH | cv::CALIB_CB_NORMALIZE_IMAGE);

    // refining pixel coordinates for given 2d points
    if (whether_found) {
      // whether select this image for calibration
      if (image_selector.addImage(image_corners)) {
        selected_file_names.push_back(file_names[i]);
        std::cout << "Select image --- " << file_names[i] << std::endl;

        // cv::imwrite(selected_image_path_ + file_names[i],
        //             cv::imread(img_dir_path_ + file_names[i]));
        cv::imwrite(selected_image_path_ + file_names[i], input_image);

        // 迭代次数通常 30~50）
        static cv::TermCriteria criteria(
            cv::TermCriteria::EPS + cv::TermCriteria::MAX_ITER, 30, 0.01);

        // cornerSubPix 要求 单通道灰度图。
        cv::cornerSubPix(gray, image_corners, cv::Size(5, 5), cv::Size(-1, -1),
                         criteria);

        // check whether the corner detect is complete
        // if (image_corners.size() == hps_.board_size.area()) {
        // for (int pt = 0; pt < image_corners.size(); pt++) {
        //   if (pt % 15 == 0) std::cout << "==\n";
        //   std::cout << pt % 15 << ": " << image_corners[pt].x << ", "
        //             << image_corners[pt].y << std::endl;
        // }

        this->addPoints(image_corners, object_corners);
        detected_image_num++;

        // display corner detect
        cv::drawChessboardCorners(input_image, hps_.board_size, image_corners,
                                  whether_found);

        cv::imwrite(detected_image_path_ + file_names[i], input_image);
        // cv::imshow(file_names[i], input_image);

        std::string name = "Corners";
        cv::namedWindow(name, cv::WINDOW_NORMAL);
        cv::resizeWindow(name, 960, 540);
        cv::imshow(name, input_image);
        cv::waitKey(50);
      }
      if (image_selector.status()) break;
    }
  }
  cv::destroyAllWindows();

  std::cout << "Calculating camera parameters..." << std::endl;

  // check min valid num of images
  if (detected_image_num < hps_.reference_img_num) {
    std::cerr << "[WARNING]Detected image num less than minium requirement.\n";
  }

  // 初始化标定结果
  this->InitCalibrationResult();

  // 标定 flags: 固定主点、纵横比，使用初值
  int flags1 = 0;
  flags1 |= cv::CALIB_USE_INTRINSIC_GUESS;  // 强先验
  flags1 |= cv::CALIB_FIX_PRINCIPAL_POINT;  // cx, cy 完全不参与优化
  flags1 |= cv::CALIB_FIX_ASPECT_RATIO;  // fx = fy
  flags1 |= cv::CALIB_ZERO_TANGENT_DIST;  // MATLAB 默认非常接近 0
  flags1 |= cv::CALIB_FIX_K3;
  flags1 |= cv::CALIB_FIX_K4 | cv::CALIB_FIX_K5 | cv::CALIB_FIX_K6;

  // 初次标定 calibrate the camera
  R_mats_.clear();
  t_mats_.clear();
  double rms = cv::calibrateCamera(object_points_, image_points_, img_size_,
                                   camera_intrinsic_, camera_dist_, R_mats_,
                                   t_mats_, flags1);
  std::cout << "[Stage 1] RMS: " << rms << std::endl;

  // 剔除异常图像 重新收集合格点
  std::vector<std::vector<cv::Point3f>> obj_pts_good;
  std::vector<std::vector<cv::Point2f>> img_pts_good;
  this->FilterPts(object_points_, image_points_, obj_pts_good, img_pts_good, 1.0);

  // 5. 重新标定
  // 允许 fx ≠ fy（轻微）
  int flags2 = 0;
  flags2 |= cv::CALIB_USE_INTRINSIC_GUESS;
  // flags2 |= cv::CALIB_FIX_PRINCIPAL_POINT;
  // flags2 |= cv::CALIB_FIX_ASPECT_RATIO;
  // flags2 |= cv::CALIB_ZERO_TANGENT_DIST;
  flags2 |= cv::CALIB_FIX_K3;
  flags2 |= cv::CALIB_FIX_K4 | cv::CALIB_FIX_K5 | cv::CALIB_FIX_K6;

  rms = cv::calibrateCamera(obj_pts_good, img_pts_good, img_size_,
                            camera_intrinsic_, camera_dist_, R_mats_, t_mats_,
                            flags2);
  std::cout << "[Stage 2] RMS = " << rms << std::endl;

  this->FilterPts(obj_pts_good, img_pts_good, obj_pts_good, img_pts_good, 1.0);

  int flags3 = 0;
  flags3 |= cv::CALIB_USE_INTRINSIC_GUESS;
  flags3 |= cv::CALIB_FIX_PRINCIPAL_POINT;
  // flags3 |= cv::CALIB_FIX_ASPECT_RATIO;
  // flags3 |= cv::CALIB_ZERO_TANGENT_DIST;
  // flags3 |= cv::CALIB_FIX_K3;
  flags3 |= cv::CALIB_FIX_K4 | cv::CALIB_FIX_K5 | cv::CALIB_FIX_K6;

  for (int iter = 0; iter < 3; ++iter) {
    rms = cv::calibrateCamera(obj_pts_good, img_pts_good, img_size_,
                              camera_intrinsic_, camera_dist_, R_mats_, t_mats_,
                              flags3);
  }
  std::cout << "[Stage 3] RMS = " << rms << std::endl;

  // 生成调优等效针孔模型
  // new_camera_intrinsic_ = cv::getOptimalNewCameraMatrix(
  //     camera_intrinsic_, camera_dist_, img_size_, 1.0, img_size_);

  // print result
  // std::cout << "\n==>CALIBRATION RESULT\n";
  std::cout << "Input " << total_image_num << " images\n";
  std::cout << "Detected " << detected_image_num << " images\n";
  std::cout << "\ncamera intrinsic: \n";
  std::cout << camera_intrinsic_.at<double>(0, 0) << "\t"
            << camera_intrinsic_.at<double>(0, 1) << "\t"
            << camera_intrinsic_.at<double>(0, 2) << std::endl;
  std::cout << camera_intrinsic_.at<double>(1, 0) << "\t"
            << camera_intrinsic_.at<double>(1, 1) << "\t"
            << camera_intrinsic_.at<double>(1, 2) << std::endl;
  std::cout << camera_intrinsic_.at<double>(2, 0) << "\t"
            << camera_intrinsic_.at<double>(2, 1) << "\t"
            << camera_intrinsic_.at<double>(2, 2) << std::endl;
  std::cout << "\ndist: \n";
  for (int i = 0; i < camera_dist_.rows; i++) {
    for (int j = 0; j < camera_dist_.cols; j++) {
      std::cout << camera_dist_.at<double>(i, j) << " ";
    }
  }
  std::cout << "\n" << std::endl;

  /*
  std::cout << "\nnew camera intrinsic: \n";
  std::cout << new_camera_intrinsic_.at<double>(0, 0) << "\t"
            << new_camera_intrinsic_.at<double>(0, 1) << "\t"
            << new_camera_intrinsic_.at<double>(0, 2) << std::endl;
  std::cout << new_camera_intrinsic_.at<double>(1, 0) << "\t"
            << new_camera_intrinsic_.at<double>(1, 1) << "\t"
            << new_camera_intrinsic_.at<double>(1, 2) << std::endl;
  std::cout << new_camera_intrinsic_.at<double>(2, 0) << "\t"
            << new_camera_intrinsic_.at<double>(2, 1) << "\t"
            << new_camera_intrinsic_.at<double>(2, 2) << std::endl;
  std::cout << "\ndist: \n";

  for (int i = 0; i < camera_dist_.rows; i++)
    for (int j = 0; j < camera_dist_.cols; j++) std::cout << 0 << ", ";
  std::cout << std::endl;
  */

  // undistort and save images
  undistortImages(selected_file_names);

  return true;
}

bool IntrinsicCalibration::undistortSingleImage(
    const std::string& image_path, const std::string& output_image_path) {
  cv::Mat input_image = cv::imread(image_path);
  if (img_size_ != input_image.size()) {
    std::cerr << "[ERROR]Input Image Size Mismatch Calibration Image Size\n";
    return false;
  }

  // 严格
  cv::initUndistortRectifyMap(camera_intrinsic_, camera_dist_, cv::Mat(),
                              camera_intrinsic_, img_size_, CV_32FC1, map1,
                              map2);
  // 调优
  // cv::initUndistortRectifyMap(camera_intrinsic_, camera_dist_, cv::Mat(),
  //                             new_camera_intrinsic_, img_size_, CV_32FC1, map1,
  //                             map2);

  cv::Mat undistorted_image;
  cv::remap(input_image, undistorted_image, map1, map2, cv::INTER_LINEAR);

  cv::imwrite(output_image_path, undistorted_image);

  return true;
}

bool IntrinsicCalibration::undistortImages(
    const std::vector<std::string>& image_names) {
  /*
  if (opendir(undistort_image_path_.c_str()) == nullptr) {
    char command[1024];
    sprintf(command, "mkdir -p %s", undistort_image_path_.c_str());
    system(command);
    printf("Create dir: %s\n", undistort_image_path_.c_str());
  }
  */
  common::CreateDir(undistort_image_path_);

  // 严格
  cv::initUndistortRectifyMap(camera_intrinsic_, camera_dist_, cv::Mat(),
                              camera_intrinsic_, img_size_, CV_32FC1, map1,
                              map2);
  // 调优
  // cv::initUndistortRectifyMap(camera_intrinsic_, camera_dist_, cv::Mat(),
  //                             new_camera_intrinsic_, img_size_, CV_32FC1, map1,
  //                             map2);

  for (int i = 0; i < image_names.size(); i++) {
    cv::Mat input_image          = cv::imread(img_dir_path_ + image_names[i]);
    std::string output_file_path = undistort_image_path_ + image_names[i];

    cv::Mat undistorted_image;
    cv::remap(input_image, undistorted_image, map1, map2, cv::INTER_LINEAR);

    cv::imwrite(output_file_path, undistorted_image);
  }

  return true;
}

void IntrinsicCalibration::SaveCalibrationResult() {
  /* way 1 opencv 4.5.0 
  cv::FileStorage fs(calib_result_file_, cv::FileStorage::WRITE);
  if (!fs.isOpened()) {
    std::cerr << "Failed to open file: " << calib_result_file_ << std::endl;
    return;
  }

  // 设置浮点输出的精度（小数位数）
  fs.precision(10);  // 默认是 6，可以改成更高精度

  fs << "camera_matrix" << camera_intrinsic_;
  fs << "dist_coeffs" << camera_dist_;
  fs.release();
  */

  // way 2
  YamlMatWriter writer(calib_result_file_);
  writer.WriteMat("camera_matrix", camera_intrinsic_);
  writer.WriteMat("dist_coeffs", camera_dist_);

  std::cout << "Saved calibration to: " << calib_result_file_ << std::endl;
}

bool IntrinsicCalibration::InitCalibrationResult() {
  // 1. 初值设定，按板格和像素近似计算 fx/fy
  // mm -> m
  // double fx_guess =
  //     img_size_.width * 0.5 / (hps_.board_size.width * hps_.grid_size * 0.001);
  // double fy_guess = fx_guess;
  // MATLAB 风格
  double fx_guess = 0.8 * img_size_.width;
  double fy_guess = fx_guess;
  double cx_guess = img_size_.width * 0.5;
  double cy_guess = img_size_.height * 0.5;

  // clang-format off
  // camera_intrinsic_ = cv::Mat::eye(3, 3, CV_64F);
  camera_intrinsic_ = (cv::Mat_<double>(3, 3) << fx_guess, 0, cx_guess, 
                                                 0, fy_guess, cy_guess, 
                                                 0, 0, 1);
  // k1,k2,p1,p2,k3,k4,k5,k6
  camera_dist_ = cv::Mat::zeros(5, 1, CV_64F);
  // clang-format on

  return true;
}

void IntrinsicCalibration::FilterPts(
    const std::vector<std::vector<cv::Point3f>>& object_points,
    const std::vector<std::vector<cv::Point2f>>& image_points,
    std::vector<std::vector<cv::Point3f>>& obj_pts_good,
    std::vector<std::vector<cv::Point2f>>& img_pts_good, double err_threshold) {
  std::vector<std::vector<cv::Point3f>> obj_pts;
  std::vector<std::vector<cv::Point2f>> img_pts;

  for (size_t i = 0; i < image_points.size(); ++i) {
    // 基本一致性检查
    if (object_points[i].empty() || image_points[i].empty() ||
        object_points[i].size() != image_points[i].size()) {
      continue;
    }

    std::vector<cv::Point2f> projected;
    cv::projectPoints(object_points[i], R_mats_[i], t_mats_[i],
                      camera_intrinsic_, camera_dist_, projected);

    // 计算重投影误差
    double err = 0;
    for (size_t j = 0; j < projected.size(); ++j) {
      err += cv::norm(projected[j] - image_points[i][j]);
    }
    err /= projected.size();

    // threshold in pixels
    if (err < err_threshold) {  // MATLAB 常用 0.5 ~ 1.0 px
      obj_pts.push_back(object_points[i]);
      img_pts.push_back(image_points[i]);
    }
  }

  obj_pts_good = obj_pts;
  img_pts_good = img_pts;
}
