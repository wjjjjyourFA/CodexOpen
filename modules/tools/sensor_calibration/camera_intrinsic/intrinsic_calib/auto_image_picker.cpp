...
AutoImagePicker::AutoImagePicker() {}

AutoImagePicker::~AutoImagePicker() {}

void AutoImagePicker::updateHyperparams(const AutoImagePickerHyperparams& hps) {
  hps_ = hps;
}

void AutoImagePicker::init(const int& img_width, const int& img_height,
  ...
  int d_imgh = static_cast<int>(hps_.IMAGE_MARGIN_PERCENT * img_height);
  int d_imgw = static_cast<int>(hps_.IMAGE_MARGIN_PERCENT * img_width);
...
bool AutoImagePicker::status() {
  if (candidate_board_.size() >= hps_.MAX_SELECTED_SAMPLE_NUM) {
...
bool AutoImagePicker::checkValidity(const BoardSquare& board) {
  if (board.area < hps_.CHESSBOARD_MIN_AREA_PORTION * img_width_ * img_height_)
    return false;  // board is too far
  if (board.angle_left_top < hps_.CHESSBOARD_MIN_ANGLE ||
      board.angle_right_top < hps_.CHESSBOARD_MIN_ANGLE ||
      board.angle_left_bottom < hps_.CHESSBOARD_MIN_ANGLE ||
      board.angle_right_bottom < hps_.CHESSBOARD_MIN_ANGLE)
...
bool AutoImagePicker::checkMoveThresh(const BoardSquare& board) {
  for (size_t i = 0; i < candidate_board_.size(); i++) {
    ...
    // 判断棋盘中心点与已有样本之间的距离
    if (dist < hps_.CHESSBOARD_MIN_MOVE_THRESH *
                   double(img_width_ + img_height_) / 2.0)
      return false;
  }
  return true;
}
...
bool AutoImagePicker::checkAreaThresh(const BoardSquare& board) {
  // 通过 getAreaScore() 判断棋盘覆盖区域是否有足够新内容：
  int score = getAreaScore(board);
  if (score <
      hps_.CHESSBOARD_MIN_AREA_CHANGE_PARTION_THRESH * img_width_ * img_height_)
    return false;
  else
    return true;
}
  ...
  for (size_t i = 0; i < candidate_board_.size(); i++) {
    ...
    if (dist < hps_.CHESSBOARD_MIN_MOVE_THRESH *
      ...
      if (fabs(board.angle_left_top - candidate.angle_left_top) <
              hps_.CHESSBOARD_MIN_ANGLE_CHANGE_THRESH &&
