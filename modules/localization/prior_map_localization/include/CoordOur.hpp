#ifndef __COORD_OUR_HPP__
#define __COORD_OUR_HPP__
...
#endif
// #define BASE_X 18705513.182095
// #define BASE_Y -1259.549668
...
inline static void GaussProjCal( double longitude, double latitude, double *X, double *Y )
{
    int ProjNo=0;
    int ZoneWide; ////带宽
    ...
    longitude0 = ProjNo * ZoneWide + ZoneWide / 2; longitude0 = longitude0 * iPI ; latitude0=0;
    ...
    xval = NN*(A+(1-T+C)*A*A*A/6+(5-18*T+T*T+72*C-58*ee)*A*A*A*A*A/120); yval = M+NN*tan(latitude1)*(A*A/2+(5-T+9*C+4*C*C)*A*A*A*A/24 +(61-58*T+T*T+600*C-330*ee)*A*A*A*A*A*A/720); X0 = 1000000L*(ProjNo+1)+500000L; Y0 = 0;

    xval = xval+X0; yval = yval+Y0; *X = xval; *Y = yval;
}

inline static void GaussProjInvCal( double X, double Y, double *longitude, double *latitude )
{
    int ProjNo;
    int ZoneWide; ////带宽

    double longitude1,latitude1, longitude0, X0,Y0, xval,yval; double e1,e2,f,a, ee, NN, T,C, M, D,R,u,fai, iPI;
    ...
    longitude0 = (ProjNo-1) * ZoneWide + ZoneWide / 2; longitude0 = longitude0 * iPI ; //中央经线
...
inline static void blh2xy(double BLH[],double INSPOS[])
{
    double x,y;
    double lat = BLH[0];
    double lon = BLH[1];
...
inline static void xy2blh(double INSPOS[],double BLH[])
{
    double lont,lat;
    double x=INSPOS[0];
    double y=INSPOS[1];
