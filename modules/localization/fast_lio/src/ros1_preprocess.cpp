...
void Preprocess::rsm1_handler(const sensor_msgs::PointCloud2::ConstPtr &msg) 
{
  pl_surf.clear();
  pl_corn.clear();
  pl_full.clear();

  pcl::PointCloud<rsm1_ros::Point> pl_orig;
  pcl::fromROSMsg(*msg, pl_orig);
  int plsize = pl_orig.points.size();
  if (plsize == 0) return;
  pl_surf.reserve(plsize);
  ...
  std::cout << "id : " << std::distance(pl_orig.begin(), min_point_it) << " " 
            << std::distance(pl_orig.begin(), max_point_it) << " "
            << pl_orig.points.size()<<endl;
  ...
  double time_start = pl_orig.points.front().timestamp;
  double time_end = pl_orig.points.back().timestamp;
  double time_diff = time_end- time_start;
  */
  ...
  {
    printf("rsm1 : timestamp field is invalid . \n");
    given_offset_time = false;
  }
  ...
  if(feature_enabled)
  {
    ...
    for (int i = 0; i < plsize; i++)
    {
      ...
      added_pt.intensity = pl_orig.points[i].intensity;
      // 不需要归一化到 0~1
      // added_pt.curvature = (pl_orig.points[i].timestamp - time_scan) * time_unit_scale; // units: ms
      added_pt.curvature = (pl_orig.points[i].timestamp - time_start) * time_unit_scale; // units: ms
