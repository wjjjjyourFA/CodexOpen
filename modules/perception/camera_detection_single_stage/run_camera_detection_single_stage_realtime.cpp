...
#include "modules/perception/camera_detection_single_stage/ros2_convert.h"
#endif

using namespace std;
using namespace jojo::perception;
using namespace jojo::perception::cdss;
namespace cdss = jojo::perception::cdss;

int main(int argc, char** argv) {
  std::string name = "ImageDetection";

  // clang-format off
  std::string config_path = "./../../../config/CameraDetection/CameraDetectionSingleStage.ini";
  std::string if_config_path = "./../../../config/CameraDetection/Interface.ini";
  // clang-format on
  ...
  auto interface_config = std::make_shared<cdss::InterfaceConfig>();
  interface_config->set_name(name);
  interface_config->LoadConfig(if_config_path);

#if defined(ENABLE_ROS1)
  ROS_INFO("\033[1;32m----> ImageDetection Started .\033[0m");
