...
int main(int argc, char** argv) {
  ...
  // clang-format off
  std::string if_config_path = "./../../../../config/PerceptionFuse/LidarCamera/InterfaceTest.ini";
  // clang-format on
  ...
  cv::Mat raw_img = cv::imread(interface_config->image_file);
  ...
  if (interface_config->b_bin_or_pcd == 0) {
    ...
    pcl::io::loadPCDFile<pcl::PointXYZI>(interface_config->lidar_file, *cloud);
  }
