...
分别表示:
  min_x坐标、min_y坐标、min_z坐标 ==> 2号点
  长度\宽度\高度 ==> 2-->1 \ 2-->3 \ 2-->6 ==> k1 k0 k2 
  min_max 对角轴 2-->4 k3
*/
//            7 -------- 4
//           /|         /|
...
struct Cube {
  ...
  Point3D<T> Center() const {
    Point3D<T> p;
    p.x = this->x + this->length / 2.0;
    p.y = this->y + this->width / 2.0;
    p.z = this->z + this->height / 2.0;
    return p;
  }

  void SetCenter(Point3D<T> p) {
    this->x = p.x - this->length / 2.0;
    this->y = p.y - this->width / 2.0;
    this->z = p.z - this->height / 2.0;
  }
...
struct BBox3D {
  ...
  Point3D<T> Center() const {
    ...
    p.z = this->zmin + (this->zmax - this->zmin) / 2.0;
    return p;
  }
