...
Eigen::Vector2f BrownCameraDistortionModel::Project(
  ...
  // 径向畸变的三次项（即 k3 * pow(r2, 3)）
  x2 = x1 * (1 + k1 * r2 + k2 * pow(r2, 2) + k3 * pow(r2, 3)) +
        2 * p1 * x1 * y1 + p2 * (r2 + 2 * x1 * x1);
  y2 = y1 * (1 + k1 * r2 + k2 * pow(r2, 2) + k3 * pow(r2, 3)) +
        p1 * (r2 + 2 * y1 * y1) + 2 * p2 * x1 * y1;
  // 二次项（即 k1 * r2 + k2 * pow(r2, 2)）
  // x2 = x1 * (1 + k1 * r2 + k2 * pow(r2, 2)) + 2 * p1 * x1 * y1 +
  //      p2 * (r2 + 2 * x1 * x1);
  // y2 = y1 * (1 + k1 * r2 + k2 * pow(r2, 2)) + p1 * (r2 + 2 * y1 * y1) +
  //      2 * p2 * x1 * y1;
  u_distorted = fx * x2 + cx;
  v_distorted = fy * y2 + cy;
  int u_d = (int)u_distorted;
  int v_d = (int)v_distorted;
  */
...
BrownCameraDistortionModel::get_camera_model() {
  ...
  auto camera_model = std::make_shared<PinholeCameraModel>();
...
bool BrownCameraDistortionModel::set_params(size_t width, size_t height,
  ...
  // std::cout << "BrownCameraDistortionModel" << std::endl;
