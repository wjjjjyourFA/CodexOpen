...
// DECLARE_string 用于声明一个字符串类型的参数。例如：
// DECLARE_string(obs_sensor_intrinsic_path) 声明了一个名为 obs_sensor_intrinsic_path 的字符串参数，
// 稍后可以通过命令行设置这个参数。
// ./your_program --obs_sensor_intrinsic_path="/path/to/intrinsic" --num_classes=10
// 在代码中，你可以通过 FLAGS_ 前缀来访问这些参数。例如：
// std::cout << "Sensor Intrinsic Path: " << FLAGS_obs_sensor_intrinsic_path << std::endl;
// std::cout << "Number of Classes: " << FLAGS_num_classes << std::endl;
...
DEFINE_string(onnx_test_input_path,
              "modules/perception/common/inference"
