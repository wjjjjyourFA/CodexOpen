...
void Eigen2MatRT(const Eigen::Matrix3d& src, cv::Mat& dst) {
  dst.create(3, 3, CV_64F);
  for (int i = 0; i < 3; ++i)
    for (int j = 0; j < 3; ++j) dst.at<double>(i, j) = src(i, j);
}

void Eigen2MatK(const Eigen::Vector3d& src, cv::Mat& dst) {
  dst.create(3, 1, CV_64F);
  for (int i = 0; i < 3; ++i) dst.at<double>(i, 0) = src(i);
}
