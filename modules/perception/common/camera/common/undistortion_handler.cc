...
bool UndistortionHandler::InitModel(const CameraDistortionModel& sensor_type) {
  ...
  switch (sensor_type) {
    case CameraDistortionModel::Brown:
      ...
      std::cout << "distort_model_name: " << distort_model->name() << std::endl;
      break;
...
bool UndistortionHandler::Init(const std::string& sensor_name) {
  ...
  switch (this->type) {
    case CameraDistortionModel::Brown:
      ...
      break;
    case CameraDistortionModel::Kannala:
      InitFisheyeUndistortRectifyMap(distort_model->get_intrinsic_params(),
                                     distort_model->get_distort_params(), 
                                     width_, height_);
...
void UndistortionHandler::InitUndistortRectifyMap(
  ...
  // std::cout << "[UndistortionHandler] InitUndistortRectifyMap() called" << std::endl;
...
#pragma omp parallel for
  ...
  for (int i = 0; i < numel; i++) {
    ...
    */
    ...
    // 像素坐标
    int u_d = static_cast<int>(fx * x_distorted + cx + 0.5);
    int v_d = static_cast<int>(fy * y_distorted + cy + 0.5);
...
void UndistortionHandler::InitFisheyeUndistortRectifyMap(
    const Eigen::Matrix3f& intrinsic_params,
    const Eigen::Matrix<float, 8, 1>& distort_params, int width, int height) {
  // transform to image coordinates
  float fx = intrinsic_params(0, 0);
