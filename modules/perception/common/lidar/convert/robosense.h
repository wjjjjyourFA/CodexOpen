...
bool PclToRsSMap128(typename pcl::PointCloud<PointT>::Ptr point_in_,
  ...
  auto& smap = GetMap128();
  // 确保目标点云大小足够
  if (point_out_->points.size() != point_in_->points.size()) {
    point_out_->points.resize(point_in_->points.size());
  }

  // sort the lidar by increasing elevation angle
  // 遍历 width 和 height，按升角索引排序
  for (int i = 0; i < point_in_->width; i++) {
    for (int j = 0; j < point_in_->height; j++) {
      point_out_->points[smap.laser_sort[j] * point_in_->width + i] =
          point_in_->points[i * point_in_->height + j];
    }
  }
