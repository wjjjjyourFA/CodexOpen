...
void DownsamplingCircularOrgAll(
  ...
  down_cloud->points.resize(cloud->size() / smp_ratio + 1);
  size_t ii = 0;
...
void DownsamplingCircularOrgPartial(
  ...
  size_t ii = 0;
  down_cloud->points.resize(smp_height * smp_width);
  all_org_idx_ptr->points.resize(smp_height * smp_width);
  ...
  down_cloud->points.resize(ii);
  all_org_idx_ptr->points.resize(ii);
}
...
void DownsamplingRectangleNeighbour(
  ...
  down_cloud->points.resize(cloud->size());
  size_t pt_num = 0;
  ...
  down_cloud->points.resize(pt_num);
}
