...
public:
    ...
    struct KD_TREE_NODE{
        ...
        // 当前子树的size 
        int TreeSize = 1;                           // 当前tree(subtree)中所有节点的个数，包含被标记为delete的节点
        // 被删除点的个数
        int invalid_point_num = 0;                  // 当前tree(subtree)中abel为delete的节点个数
        int down_del_num = 0;
        // 该点被删除的标签
        bool point_deleted = false;                 // 当前节点的label
        // 该子树被删除的标签
        bool tree_deleted = false;                  // 整个tree的label
        ...
        // 该子树的点张成的包络框两个顶点
        float node_range_x[2], node_range_y[2], node_range_z[2];   // 当前tree对应的外包络盒
        // 左右子节点
