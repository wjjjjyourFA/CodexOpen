...
void CameraLocationEstimation::DetectionAndLocation(cv::Mat& cimage,
  ...
  /* way 1  // default local_track_id = -1
...
void CameraLocationEstimation::FrameObjectGetObjRec(
  ...
  for (auto& d : detections) {
    ...
    // way 1 legacy // FrameObject 继承自 base::Object
    // FrameObject fb;  // frame_box
    // static_cast<base::Object&>(fb) = d;
    ...
    */
    ...
    // 20240721 问题，收缩框一直偏左。实际应该是向中间收缩。
    // clang-format off
    // cv::Rect cen_rec((int)(rec.x + 0.35 * rec.width), (int)(rec.y + 0.35 * rec.height), 
    //                  (int)(0.3 * rec.width), (int)(0.3 * rec.height));
    // clang-format on

    // 20251016 更正，向中间收缩，并添加对图像边缘的兼容
...
void CameraLocationEstimation::GetCloudAndCluster(
  ...
  // cv::split 会自动将 mask 分割成多个单通道图像，并将它们存储在 splits 中。
  // 如果 mask 是一个三通道图像（如 RGB 图像），BGR 顺序（OpenCV 默认），则 splits 会包含三个单通道图像。
  // splits[0] [1] [2] 分别是 B G R
  // 投影 mask 是三通道的 Mat r g b => 雷达坐标系的点 x y z
  cv::split(mask, splits);
  // std::cout<<"splits.size(): "<<splits.size()<<std::endl;
  /* 已证明，可以正常传值，即使 有负值，只是显示异常，但值仍旧存在Mat中
  cv::imshow("splits[0]", splits[0]);
  cv::imshow("splits[1]", splits[1]);
  cv::imshow("splits[2]", splits[2]);
  */
  ...
  // 预分配内存空间
  // splits_roi.reserve(3);
  // 清空 vector，但保留内存，以便之后添加元素时不必重新分配内存。
  // splits_roi.resize(0);
  // 做并行计算，并且固定 3 个位置存放 B/G/R；创建 3 个空的 cv::Mat 默认对象
  splits_roi.resize(3);
...
void CameraLocationEstimation::Cluster(
  ...
  */
  ...
  /* way 1 满足阈值的聚类，手动调整结果
  std::vector<std::shared_ptr<base::Segment>> SegVector;
  if (object_cluster->GetOutputSegs(SegVector)) {
    // 按不同的策略选择聚类 default : Maximum cluster
    object_cluster->ClusterPolicy(SegVector, result, 1);
  }
  */
