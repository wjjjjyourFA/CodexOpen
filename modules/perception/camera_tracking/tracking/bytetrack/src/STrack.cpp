...
STrack::STrack(vector<float> tlwh_, float score, int label) : STrack(tlwh_, score)
{  
	// add by fszn jojo
	this->label = label;
	if (label >= 0) {
...
void STrack::activate(bytetracker::KalmanFilter &kalman_filter, int frame_id)
{
...
void STrack::re_activate(STrack &new_track, int frame_id, bool new_id)
{
	...
	if (new_track.label >= 0) {
		label_hist[new_track.label]++;
	}
}
...
void STrack::multi_predict(vector<STrack*> &stracks, bytetracker::KalmanFilter &kalman_filter)
{
