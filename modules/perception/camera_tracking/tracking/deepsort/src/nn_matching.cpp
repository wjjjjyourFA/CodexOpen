  ...
  for (TRACKER_DATA & data:tid_feats) {
        ...
        if (samples.find(track_id) != samples.end()) {  //append
            ...
            if (newSize <= this->budget) {
                ...
                newSampleFeatures.block(0, 0, oldSize, kFeatureDim) = samples[track_id];
                ...
                if (oldSize < this->budget) {   //original space is not enough;
                    FEATURESS newSampleFeatures(this->budget, kFeatureDim);
                    if (addSize >= this->budget) {
                        newSampleFeatures = newFeatOne.block(0, 0, this->budget, kFeatureDim);
                    } else {
                        newSampleFeatures.block(0, 0, this->budget - addSize, kFeatureDim) =
                            samples[track_id].block(addSize - 1, 0, this->budget - addSize, kFeatureDim).eval();
                        newSampleFeatures.block(this->budget - addSize, 0, addSize, kFeatureDim) = newFeatOne;
                    }
                    samples[track_id] = newSampleFeatures;
                } else {        //original space is ok;
                    if (addSize >= this->budget) {
                        samples[track_id] = newFeatOne.block(0, 0, this->budget, kFeatureDim);
                    } else {
                        samples[track_id].block(0, 0, this->budget - addSize, kFeatureDim) =
                            samples[track_id].block(addSize - 1, 0, this->budget - addSize, kFeatureDim).eval();
                        samples[track_id].block(this->budget - addSize, 0, addSize, kFeatureDim) = newFeatOne;
                    }
                }
            }
