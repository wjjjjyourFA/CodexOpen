...
class KalmanFilter {
 public:
   ...
   */
  KalmanFilter() {
    F_.setIdentity();  // 状态转移矩阵（State Transition Matrix）
    Q_.setZero();      // 过程噪声协方差（Process noise covariance）
    H_.setIdentity();  // 观测矩阵（Observation matrix）
    R_.setZero();      // 观测噪声协方差（Measurement noise covariance）
    B_.setZero();      // 控制输入矩阵（Control matrix）

    x_.setZero();      // 当前状态估计（Posterior state estimate）
    P_.setZero();      // 当前状态协方差（Posterior covariance）
...
inline void KalmanFilter<T, XN, ZN, UN>::Correct(
  ...
  const Eigen::Matrix<T, XN, XN> identity_minus_gain_observation =
      Eigen::Matrix<T, XN, XN>::Identity() - K_ * H_;
  P_ = identity_minus_gain_observation * P_ *
           identity_minus_gain_observation.transpose() +
       K_ * R_ * K_.transpose();
  P_ = static_cast<Eigen::Matrix<T, XN, XN>>(
      (P_ + P_.transpose()) * static_cast<T>(0.5));
}
