...
/**
 * @file
 * @brief Defines the EulerAnglesXYZ class.
 */
 ...
 * Our CodexOpen reference frame follows ROS's convention:
 * Forward/Left/Up (FLU) respectively for the axes x/y/z.
 * In particular, we describe the orientation of the car by three angles:
 * 1) the roll, in [-pi, pi), corresponds to a rotation around the x-axis;
 * 2) the pitch, in (-pi/2, pi/2), corresponds to a rotation around the y-axis;
 ...
 * (横滚角为零表示车辆水平，正值表示左侧上升)
 * The pitch is zero when the car is level and positive when the nose is down.
 * (俯仰角为零表示车辆水平，正值表示车头向下)
 * The yaw is zero when the car is facing North, and positive when facing West.
 * (偏航角为零表示车辆朝北，正值表示从上方观察为逆时针旋转，即左转)
 * In turn, in the world frame, the x/y/z axes point to East/North/Up (ENU).
 * (在世界坐标系（ENU）中，X/Y/Z 轴分别指向东/北/上)
...
class EulerAnglesXYZ {
 public:
   ...
   */
  EulerAnglesXYZ() : roll_(0), pitch_(0), yaw_(0) {}
   ...
   */
  explicit EulerAnglesXYZ(T yaw) : roll_(0), pitch_(0), yaw_(yaw) {}
   ...
   * @param yaw The yaw of the car
   */
  EulerAnglesXYZ(T roll, T pitch, T yaw)
