...
bool Ros1Convert::Init(ros::NodeHandle &nh, ros::NodeHandle &private_nh,
  ...
  for (auto config : gnss_configs) {
    ...
    driver.gnss_puber =
        nh_.advertise<custom_sensor_msgs::Gnss>(topic + "/gps_data", 10);

    driver.hb_puber =
        nh_.advertise<monitor_msgs::HeartBeat>(topic + "/hb", 10);

    if (driver.conf.pub_imu) {
      driver.imu_puber =
          nh_.advertise<custom_sensor_msgs::Imu>(topic + "/imu_data", 10);
    }
