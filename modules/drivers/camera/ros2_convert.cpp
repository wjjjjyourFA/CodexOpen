...
// 这里指的是CV格式的图像
void Ros2Convert::image_pub(SensorConfig& config, DriverWrapper& driver) {
  ...
  driver.ros2_msg =
      cv_bridge::CvImage(header, "bgr8", output_image).toImageMsg();
...
// 这里指的是sensor_msgs::msg::CompressedImage格式的图像
void Ros2Convert::image_compressed_pub() {}

bool Ros2Convert::Init(std::shared_ptr<drivers::ConfigManager> param) {
  ...
  for (auto config : camera_configs) {
    ...
    node->declare_parameter<int>(param_topic + ".jpeg_quality",
