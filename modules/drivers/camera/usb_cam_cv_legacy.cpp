...
int UsbCamCv::init_mjpeg_decoder(int image_width, int image_height) {
  ...
  // way 2
  avframe_camera_size_ = av_image_get_buffer_size(AV_PIX_FMT_YUV422P,
...
bool UsbCamCv::poll(const CameraImagePtr& raw_image) {
  ...
  r = select(fd_ + 1, &fds, nullptr, &eset, &tv);
...
bool UsbCamCv::init_device(void) {
  ...
  // 逐行扫描：现代摄像头 =|= 机器视觉 / 自动驾驶 / SLAM / perception
...
bool UsbCamCv::init_userp(unsigned int buffer_size) {
  ...
  req.count  = V4L2_BUFFER_LENGHT;  // 4
...
bool UsbCamCv::read_frame(CameraImagePtr raw_image) {
  ...
  switch (config_->io_method()) {
    ...
    case IO_METHOD_MMAP:
      ...
      // std::cout << "raw_image->height: " << raw_image->height << std::endl;
...
bool UsbCamCv::process_image(void* src, int len, CameraImagePtr dest,
  ...
  if (pixel_format_ == V4L2_PIX_FMT_YUYV ||
    ...
    // 0拷贝
    // 创建一个 Mat 头部，不分配内存，直接指向 data 指针
    cv::Mat yuv(config_->height(), config_->width(), CV_8UC2, data);
    ...
    */
    ...
    if (pixel_format_ == V4L2_PIX_FMT_YUYV) {
      cv::cvtColor(yuv, dest->image, cv::COLOR_YUV2BGR_YUYV);
      // cv::cvtColor(yuv, dest->image, cv::COLOR_YUV2BGR_UYVY);
      // std::cout << "is yuyv" << std::endl;
      if (show) {
        cv::imshow("yuyv", dest->image);
        cv::waitKey(1);
      }
      ...
      cv::cvtColor(yuv, dest->image, cv::COLOR_YUV2BGR_UYVY);
      // cv::cvtColor(yuv, dest->image, cv::COLOR_YUV2BGR_YUYV);
      // std::cout << "is uyvy" << std::endl;
      if (show) {
        cv::imshow("uyvy", dest->image);
        cv::waitKey(1);
      }
    }
    ...
    // assert(dest->yuv_image.total() * dest->yuv_image.elemSize() >= capture_buf_size);
    // memcpy(dest->yuv_image.data, src, capture_buf_size);
    // std::cout << "yuv_image rows=" << dest->yuv_image.rows
    //           << " cols=" << dest->yuv_image.cols
    //           << " step=" << dest->yuv_image.step
    //           << " expected_step=" << dest->yuv_image.cols * 2
    //           << std::endl;
    // std::cout << "image rows=" << dest->image.rows
    //           << " cols=" << dest->image.cols
    //           << " type=" << dest->image.type()
    //           << std::endl;
    // std::cout << "capture_buf_size: " << capture_buf_size << std::endl;
    memcpy(dest->yuv_image.data, data, len);
  } else {
...
#ifdef __aarch64__
int UsbCamCv::convert_yuv_to_rgb_pixel(int y, int u, int v) {
...
int UsbCamCv::convert_yuv_to_rgb_buffer(unsigned char* yuv, unsigned char* rgb,
                                        unsigned int width,
                                        unsigned int height) {
