...
void SuperLIOReLoc::requestParamFileInit()
{
  use_param_init_request_.store(true);
}
...
bool SuperLIOReLoc::kf_init(){
  ...
  if (use_param_init_request_.exchange(false)) {
    re_init_pose_ = param_init_pose_;
    flg_get_init_guess_ = true;
    has_init_trigger_ = true;
    LOG(INFO) << YELLOW
              << " ---> Use init_pose from param file after Enter key."
              << RESET;
  }
  ...
  if (!has_init_trigger_) {
    return false;
  }

  /// A new init pose was provided. Reset the accumulated data and restart initialization.
  if(flg_get_init_guess_){
    imu_cout = 0;
    init_frame_count = 0;
