...
bool BuildTimestampedMapPath(std::filesystem::path& output_map_path) {
  const std::filesystem::path map_directory(g_save_map_dir);
  std::error_code error;
  std::filesystem::create_directories(map_directory, error);
  if (error) {
    LOG(ERROR) << RED << " ---> Failed to create map directory: "
               << map_directory << ", error: " << error.message() << RESET;
    return false;
  }
  ...
  if (localtime_r(&now, &local_time) == nullptr) {
    LOG(ERROR) << RED << " ---> Failed to get local time for map filename"
               << RESET;
    return false;
  }

  std::ostringstream timestamp;
  timestamp << std::put_time(&local_time, "%m%d%H");
  std::string map_stem = std::filesystem::path(g_map_name).stem().string();
  if (map_stem.empty()) {
    map_stem = "map";
  }
  output_map_path =
      map_directory / (map_stem + "_" + timestamp.str() + ".pcd");
  return true;
}
...
void SuperLIO::caceData(){
  ...
  if(!rm_PCD_dir){
    ...
    const std::filesystem::path pcd_directory =
        std::filesystem::path(g_save_map_dir) / "PCD";
    std::error_code error;
    std::filesystem::remove_all(pcd_directory, error);
    if (error) {
      LOG(ERROR) << RED << " ---> Failed to reset PCD directory: "
                 << pcd_directory << ", error: " << error.message() << RESET;
      return;
    }
    std::filesystem::create_directories(pcd_directory, error);
    if (error) {
      LOG(ERROR) << RED << " ---> Failed to create PCD directory: "
                 << pcd_directory << ", error: " << error.message() << RESET;
      return;
    }
  }
...
void SuperLIO::ProcessCaceMap(){
  ...
  fs::path output_map_path;
  if (!BuildTimestampedMapPath(output_map_path)) return;
  const std::string output_map_name = output_map_path.string();
...
void SuperLIO::saveMap(){
  ...
  if(!point_map_->empty()){
    ...
    if (!BuildTimestampedMapPath(output_map_path)) return;
    const std::string map_name = output_map_path.string();
...
void SuperLIO::Output(){
  ...
  bool publish_visual_map = false;
  if (g_visual_map) {
    static int count = -1;
    count++;
    if (count % g_pub_step == 0) {
      count = 0;
      publish_visual_map = true;
    }
  }

  if (!publish_visual_map && !g_ax7_compat_output) {
    return;
  }
  ...
  if (publish_visual_map) {
    data_wrapper_->pub_cloud_world(world_pc, state.timestamp);
  }
  if (g_ax7_compat_output) {
    data_wrapper_->pub_registered_scan(world_pc, state.timestamp);
  }
}
