...
public:
  SuperLIOReLoc(){
    ...
    param_init_pose_ = BASIC::SE3(BASIC::SO3(init_R2), init_t);
    re_init_pose_ = param_init_pose_;
  };
  ~SuperLIOReLoc(){};

  void init() override;
  void requestParamFileInit();
...
private:
  ...
  bool has_init_trigger_ = false;
  BASIC::SE3 re_init_pose_;
  BASIC::SE3 param_init_pose_;
  std::atomic<bool> use_param_init_request_{false};
};
