...
# QT5_REAL_PREFIX="${THIRD_PARTY_DIR}/Qt-${VERSION}"
QT5_REAL_PREFIX="/usr/local/Qt-${VERSION}"

# Qt 官方并不提供“可直接 tar 解压使用的通用 ARM64 Linux Qt5 二进制包”。
# 自己用安装器安装一次后，将其打包，从而可以在其他的 arm 版本上使用
if [[ "${BUILD_TYPE}" == "download" ]]; then
    ...
    pushd qtbase-everywhere-src-${VERSION} >/dev/null
        ...
        # 绝对路径
        # ln -sfnv "${QT5_REAL_PREFIX}" "${QT5_PREFIX}"
        # 相对路径
        cd "${QT5_REAL_PREFIX}/../"
        ln -sfnv "Qt-${VERSION}" "qt5"
...
if [[ "${BUILD_TYPE}" == "build" ]]; then
    ...
    # 在“源码构建场景”里，这些通常本来就不存在
    # rm -rf ${QT5_PREFIX}/{Docs,Examples,Tools,dist} || true
    # rm -rf ${QT5_PREFIX}/MaintenanceTool* || true
    # rm -rf ${QT5_PREFIX}/{InstallationLog.txt,installer-changelog} || true
    # rm -rf ${QT5_PREFIX}/{components,network}.xml || true
else
