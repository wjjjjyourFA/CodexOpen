...
if [ ! -d "osqp-${OSQP_VER}" ]; then
git clone --recursive --depth 1 --branch v${OSQP_VER} \
    https://github.com/osqp/osqp.git \
    "${THIRD_PARTY_DIR}/osqp-${OSQP_VER}"
fi

if [ ! -d "qdldl-${QDLDL_VER}" ]; then
git clone --recursive --depth 1 --branch v${QDLDL_VER} \
    https://github.com/osqp/qdldl.git \
    "${THIRD_PARTY_DIR}/qdldl-${QDLDL_VER}"
fi
...
pushd "osqp-${OSQP_VER}"
    ...
    # rm -rf ${PKG_NAME}
    cp -a ./../qdldl-${QDLDL_VER}/* ./lin_sys/direct/qdldl/qdldl_sources/
