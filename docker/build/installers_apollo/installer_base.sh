...
# export PKGS_DIR="/opt/CodexOpen/pkgs"
...
function apollo_environ_setup() {
    ...
    # if [ ! -f "${CODEXOPEN_LD_FILE}" ]; then
    if ! grep -qx "${SYSROOT_DIR}/lib" "${CODEXOPEN_LD_FILE}" 2>/dev/null; then
        echo "${SYSROOT_DIR}/lib" | tee -a "${CODEXOPEN_LD_FILE}"
    fi

    local log_dir
    log_dir="$(dirname "${DOWNLOAD_LOG}")"
    mkdir -p "${log_dir}"
