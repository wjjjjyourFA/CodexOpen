...
# 6. mkdir libtorch_gpu && cp -r include libtorch_gpu/ && cp -r lib libtorch_gpu/ && mv libtorch_gpu /usr/local/

bash ${CURR_DIR}/install_mkl.sh

TARGET_ARCH="$(uname -m)"

apt update && apt install -y libopenblas-base libopenmpi-dev libomp-dev
