...
bool Writer<MessageT>::Init() {
  std::cout << "Writer<MessageT>::Init()" << std::endl;
  {
...
bool Writer<MessageT>::Write(const std::shared_ptr<MessageT>& msg_ptr) {
  RETURN_VAL_IF(!WriterBase::IsInit(), false);
  std::cout << "Writer start write msg " << std::endl;
  return transmitter_->Transmit(msg_ptr);
}

template <typename MessageT>
void Writer<MessageT>::JoinTheTopology() {
  std::cout << "Writer<MessageT>::JoinTheTopology()" << std::endl;
  ...
  for (auto& reader : readers) {
    ...
    std::cout << "Writer<MessageT>::JoinTheTopology() : transmitter_->Enable(reader)" << std::endl;
  }
