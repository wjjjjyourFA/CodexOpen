...
bool GetProtoFromASCIIFile(const std::string& file_name,
  ...
  // std::cout << "GetProtoFromASCIIFile" << std::endl;
  using google::protobuf::TextFormat;
...
bool GetProtoFromBinaryFile(const std::string& file_name,
                            google::protobuf::Message* message) {
  // std::cout << "GetProtoFromBinaryFile" << std::endl;
...
// 判断文件名是否以指定后缀结尾
bool HasSuffix(const std::string& file_name, const std::string& suffix) {
  if (file_name.length() < suffix.length()) return false;
  return file_name.compare(file_name.length() - suffix.length(),
                           suffix.length(), suffix) == 0;
}

std::vector<std::string> GetFilesWithExtension(const std::string& folder_path,
  ...
  while ((entry = readdir(dir)) != nullptr) {
    ...
    if (entry->d_type == DT_REG) {  // 普通文件
      ...
      if (HasSuffix(filename, extension)) {
        // files.push_back(folder_path + "/" + filename);  // 保存完整路径
        files.push_back(filename);  // 只保存文件名
      }
    }
  }
