...
// 允许一个类的成员通过 std::shared_ptr 获得指向自己类实例的智能指针
/*
  // 使用 shared_ptr 创建 CCObjectPool 的实例
  std::shared_ptr<CCObjectPool<int>> pool = std::make_shared<CCObjectPool<int>>();
  
  // 获取指向当前对象的 shared_ptr
  std::shared_ptr<CCObjectPool<int>> poolCopy = pool->getSharedPtr();

  poolCopy->show();  // 调用成员函数
*/
// 通过智能指针管理对象的生命周期，并且避免多个创建，也不会触发多次销毁
template <typename T>
